    ...
    <div>
      ...
      <Form.Item
        ...
        <Input.Group compact style={{display:'flex', alignItems:'center'}}>
          <label style={{width: "30%"}}>Amount to withdraw: </label>
          <Input placeholder={"USDC"} value={htAmount} onChange={event => setHtAmount(event.currentTarget.value)} style={{ width: "50%" }} />
          <Button style={{border: "1px solid rgb(67, 67, 67)", width: "20%", padding:0}} onClick={e => setHtAmount(((htBalance + lockedLiquidity) / htSupply * userHT / LAMPORTS_PER_HT).toFixed(2).toString())} disabled={userHT === 0}>MAX</Button>
        </Input.Group>
      </Form.Item>
      <WalletSlider 
      ...
      <div style={{display:'flex', justifyContent:'space-between', marginTop:'2.5em'}}>
        ...
        <Button onClick={onWithdraw} disabled={Number(htAmount) === 0} style={{width:'48%', padding: '0.6em', height:'auto'}}>
          <LinkLabel style={{ margin:"auto" }}>
            <span>Withdraw</span>
          </LinkLabel>
        </Button>
      ...
      <Form.Item name="usdcAmount" style={{marginBottom:'1em'}}>
        <Input.Group compact style={{display:'flex', alignItems:'center'}}>
          <label style={{width: "30%"}}>Amount to unstake: </label>
          <Input placeholder={"USDC"} name="usdcAmount" style={{width: "50%"}} />
